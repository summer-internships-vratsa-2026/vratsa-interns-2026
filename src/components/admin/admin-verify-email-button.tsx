...
import { verifyAdminUserEmailAction } from "@/actions/admin-users";
...
type AdminVerifyEmailButtonProps = {
  locale: string;
  userId: string;
  variant?: "default" | "outline";
  size?: "default" | "sm";
  labelKey?: "markVerified" | "confirmEmail";
  showFeedback?: boolean;
};

export function AdminVerifyEmailButton({
  locale,
  userId,
  variant = "outline",
  size = "default",
  ...
  useEffect(() => {
    if (state.success === "email_verified") {
      setConfirming(false);
      router.refresh();
    }
    ...
    <div className="inline-flex flex-col gap-2">
      ...
      {confirming ? (
        <div className="flex max-w-sm flex-col gap-2 rounded-md border border-border bg-background/80 p-3 text-left">
          <p className="text-sm text-muted-foreground">{t("confirmEmailPrompt")}</p>
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isPending ? t("loading") : t("confirmEmailAction")}
            </Button>
          </div>
        </div>
      ) : (
